...
// 启动时：为已有设备初始化默认值 & LoRa 映射 & EID 映射
...
// 内部小工具：完全复用你 AddDevice 里的那一坨逻辑（除了 EID）
func initOneDevice(deviceName string, prof models.DeviceProfile, lc logger.LoggingClient) {
	for _, dr := range prof.DeviceResources {
		...
		if err := config.DeviceInit(deviceName, resName, defaultValue, valueType); err != nil {
			lc.Warnf("InitDeviceValues: 初始化设备 %s 资源 %s 失败：%v", deviceName, resName, err)
			continue
		}
		lc.Debugf("InitDeviceValues: 已将设备 %s 的资源 %s 初始化: %s (类型: %s)",
		...
		if dr.Attributes == nil {
			lc.Debugf("InitDeviceValues: 资源 %s 未配置 attributes，跳过 LoRa 登记", resName)
			continue
		}

		rawLora, ok := dr.Attributes["lora"]
		if !ok || rawLora == nil {
			lc.Debugf("InitDeviceValues: 资源 %s 未配置 attributes.lora，跳过 LoRa 登记", resName)
			continue
		}

		loraMap, ok := rawLora.(map[string]any)
		if !ok {
			lc.Warnf("InitDeviceValues: 资源 %s attributes.lora 类型异常: %T，期望 map[string]any，跳过 LoRa 登记",
				resName, rawLora)
			continue
		}
		...
		if featStr == "" || typeStr == "" {
			lc.Debugf("InitDeviceValues: 资源 %s 的 lora.paramFeatures 或 lora.paramType 为空，跳过 LoRa 登记", resName)
			continue
		}
		...
		lc.Debugf("InitDeviceValues: ParamEidRegistry 登记: dev=%s res=%s -> Feature=%03b Code=%011b",
